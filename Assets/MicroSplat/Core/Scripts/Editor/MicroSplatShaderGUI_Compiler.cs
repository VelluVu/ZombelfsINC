...
public partial class MicroSplatShaderGUI : ShaderGUI
{
   ...
   public class MicroSplatCompiler
   {
      ...
      void WriteProperties(string[] features, StringBuilder sb, bool blendable)
      {
         ...
         {
            sb.AppendLine("      [HideInInspector] _Control4 (\"Control4\", 2D) = \"black\" {}");
         }
         if (max24 || max28 || max32)
         {
            sb.AppendLine("      [HideInInspector] _Control5 (\"Control5\", 2D) = \"black\" {}");
         }
         if (max28 || max32)
         {
            sb.AppendLine("      [HideInInspector] _Control6 (\"Control6\", 2D) = \"black\" {}");
         }
         if (max32)
         {
            sb.AppendLine("      [HideInInspector] _Control7 (\"Control7\", 2D) = \"black\" {}");
         }
